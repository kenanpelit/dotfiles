...
complete -c qemu-system-rx -o hdd -d 'Use file as hard disk 0, 1, 2 or 3 image (see the \\%Disk Images chapter in th…'
complete -c qemu-system-rx -o cdrom -d 'Use file as CD-ROM image (you cannot use -hdc and -cdrom at the same time)'
...
complete -c qemu-system-rx -o no-hpet -d 'Disable HPET support'
...
complete -c qemu-system-rx -o mon -d 'Setup monitor on chardev name'
...
complete -c qemu-system-rx -o singlestep -d 'Run the emulation in single step mode'
...
complete -c qemu-system-rx -o chroot -d 'Immediately before starting guest execution, chroot to the specified directory'
...
complete -c qemu-system-rx -o semihosting-config -d 'Enable and configure semihosting (ARM, M68K, Xtensa, MIPS, Nios II, RISC-V on…'
...
complete -c qemu-system-rx -o async-teardown -d 'Enable asynchronous teardown'
